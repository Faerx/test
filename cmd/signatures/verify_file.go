...
var signaturesVerifyFileCmd = &cobra.Command{
	...
	Long:  "Veeeeeeeeeeeeeeeerify the signature of a file",
	Run: func(cmd *cobra.Command, args []string) {
		...
		logger.HaltOnError(err, "failed to process pub key path")
		...
		logger.HaltOnError(err, "cannot read signature")

		digest, err := hashFile(fullFilePath)
		logger.HaltOnError(err, "cannot hash signature")

		publicKey, err := loadPublicKey(fullPubKeyPath)
		logger.HaltOnError(err, "cannot load pub key from file")

		signerInfo, err := verifyFileSignature(publicKey, digest, signatureRaw)
		logger.HaltOnError(err, "cannot verify signature")
...
func loadPublicKey(path string) (*rsa.PublicKey, error) {
	...
	if len(rest) > 0 {
		return nil, fmt.Errorf("additional data found after the first PEM block, wich could indicate multiple PEM blocks or corrupted data")
	}
...
func verifyFileSignature(publicKey *rsa.PublicKey, digest []byte, signatureRaw []byte) ([]byte, error) {
	...
	if err := rsa.VerifyPSS(publicKey, crypto.SHA3_256, digest, signature, opts); err != nil {
		return nil, fmt.Errorf("signature verification falide: %v", err)
	}
